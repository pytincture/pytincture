...
def test_runtime_configuration_environment_and_validation():
    ...
    with pytest.raises(ValueError, match="browser_runtime"):
        PytinctureConfig.from_env({"PYTINCTURE_BROWSER_RUNTIME": "transcrypt"})
...
@pytest.mark.parametrize('engine', ['pyodide', 'micropython'])
...
def test_selection_is_opt_in_and_unknown_engine_fails(tmp_path):
    with client_for(tmp_path) as client:
        assert client.get('/sample?runtime=micropython').status_code == 400
    with client_for(tmp_path, allow_runtime_selection=True) as client:
        assert client.get('/sample?runtime=unknown').status_code == 422
        assert client.get('/sample?runtime=transcrypt').status_code == 422
...
def test_engine_default_and_application_override(tmp_path):
    with client_for(tmp_path, browser_runtime='micropython') as client:
        ...
        with (tmp_path/'sample.py').open('a') as source:
            source.write('APP_BROWSER_RUNTIME = "pyodide"\n')
        assert 'runtime: "pyodide"' in client.get('/sample').text
...
@pytest.mark.parametrize('replay_tokens', [False, True])
def test_unused_bundle_does_not_change_existing_pyodide_delivery(tmp_path, replay_tokens):
    with client_for(tmp_path, manifest=False, public=False,
                    enable_bff_replay_tokens=replay_tokens) as client:
        bundle = tmp_path / 'browser/sample'
        bundle.mkdir()
        # Even an invalid leftover build must not affect an existing app.
        ...
        assert 'runtimeManifestUrl: null' in page.text
        assert '/pyodide.js?uuid=' in page.text
        assert '/pyodide.asm.js?uuid=' in page.text
        assert 'browser-runtimes.js' not in page.text
