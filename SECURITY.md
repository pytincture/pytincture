...
| SR-03 BFF pre-admission buffering | Remediated in [#320](https://github.com/pytincture/pytincture/issues/320). Every uncached body acquires bounded per-peer/per-worker upload admission before its first receive; queued calls do not buffer bodies. | Upload admission is released before execution admission and remains separately configurable from execution timeout, async capacity, sync calls, and streaming. |
| SR-04 appcode download lifetime | Remediated in [#321](https://github.com/pytincture/pytincture/issues/321) with full-response admission, write/total deadlines, and optional atomic prebuilt production `appcode.pyt`. | Dynamic development remains supported and backend code remains installed separately. |
| SR-05 optional revocation-store reads | Remediated in [#322](https://github.com/pytincture/pytincture/issues/322). Signed-cookie validation is local; every enabled shared revocation read uses bounded async admission, a deadline, off-loop execution, and the provider circuit breaker. | Remote state remains optional; Redis is not required. |
| SR-06 SAML initiation/metadata work | Remediated in [#323](https://github.com/pytincture/pytincture/issues/323). Public SAML routes rate-limit peers, prove the application exists before provider work, and run toolkit/crypto work behind dedicated bounded off-loop admission. Metadata uses a bounded settings-fingerprint cache. | SAML remains stateless and load-balancer friendly. |
| SR-07 CSRF cookie alias shadowing | Remediated in [#324](https://github.com/pytincture/pytincture/issues/324). The hosted page supplies one validated cookie name and JavaScript logging plus generated BFF clients read only that exact cookie. | Production accepts only the `__Host-` cookie; local HTTP development keeps its separate development cookie. Prebuilt appcode remains portable. |
| SR-08 browser request cancellation | Remediated in [#325](https://github.com/pytincture/pytincture/issues/325). Generated async calls attach an `AbortController`, abort on timeout/cancellation, and cancel stream readers in cleanup paths. | Sync, async, streaming, and `yield` APIs remain available; only abandoned browser work ends sooner. |
| SR-09 service-worker cache amplification | Remediated in [#326](https://github.com/pytincture/pytincture/issues/326). Manifest assets canonicalize to pathname plus the one instance UUID, and activation removes polluted/current and stale cache entries only inside the same application namespace. | Supported instance UUID/version cache busting and clean navigation URLs remain; other applications and foreign caches are untouched. |
| SR-10 warm appcode validation scan | Remediated in [#327](https://github.com/pytincture/pytincture/issues/327). Warm hits stat only a bounded relevant source/directory fingerprint under a configurable deadline; cold discovery applies the existing file, byte, directory, and scanned-file limits. | Relevant same-name edits and new matching files still invalidate disposable per-worker caches; production prebuilds avoid runtime discovery. |
| SR-11 replay-refill waiter overflow | Remediated in [#328](https://github.com/pytincture/pytincture/issues/328). Waiters share a cancellation-shielded single-flight refill, recheck the pool, and start another batch only when needed. Empty and failed refills produce controlled errors. | Async concurrency is not capped; callers may span multiple configured batches, and cancellation of one caller does not cancel work shared by others. |
| SR-12 isolated-process setup cleanup | Remediated in [#329](https://github.com/pytincture/pytincture/issues/329). Pipe creation, process construction, and partial start all run under exactly-once admission cleanup; partial handles close and children terminate/reap. Defaults are eight per worker and four per stable identity. | Only optional isolated execution changes. Operators needing the prior resource envelope can explicitly set `BFF_ISOLATED_MAX_CONCURRENCY=4` and `BFF_ISOLATED_MAX_PER_USER=2`; trusted execution and stateless load balancing are unchanged. |
