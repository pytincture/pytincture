...
def _nginx_proxy_checks(config: str, expected_host: str) -> dict[str, bool]:
    without_comments = "\n".join(line.split("#", 1)[0] for line in config.splitlines())
    normalized = re.sub(r"\s+", " ", without_comments).strip()
    escaped_host = re.escape(expected_host)
    return {
        "server_name": bool(
            re.search(rf"\bserver_name\s+[^;]*\b{escaped_host}\b[^;]*;", normalized)
        ),
        "upstream_host_replaced": bool(
            re.search(r"\bproxy_set_header\s+Host\s+\$host\s*;", normalized, re.I)
        ),
        "forwarded_host_replaced": bool(
            re.search(
                r"\bproxy_set_header\s+X-Forwarded-Host\s+\$host\s*;",
                normalized,
                re.I,
            )
        ),
        "forwarded_proto_replaced": bool(
            re.search(
                r"\bproxy_set_header\s+X-Forwarded-Proto\s+\$scheme\s*;",
                normalized,
                re.I,
            )
        ),
        "caller_forwarded_values_rejected": not bool(
            re.search(
                r"proxy_set_header\s+(?:Host|X-Forwarded-Host|X-Forwarded-Proto)\s+"
                r"\$(?:http_)?(?:x_)?forwarded_",
                normalized,
                re.I,
            )
        ),
...
def build_evidence(
    ...
    *,
    ...
    proxy_checks = _nginx_proxy_checks(
        proxy_config.decode("utf-8"), urlsplit(https_origin).hostname or ""
    )
    ...
    checks = {
        ...
        "trusted_proxy_headers": canonical_origin_ok and all(proxy_checks.values()),
