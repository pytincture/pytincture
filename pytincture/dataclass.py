...
"""
    ...
    for class_node in class_nodes:
        ...
        if any(
            ...
            stub_class_code += "            if separator and name == cookie_name:\n"
            ...
            stub_class_code += "        tokens = self._decode_pytincture_state(str(req.responseText))\n"
            stub_class_code += "        if not tokens:\n"
            stub_class_code += "            raise RuntimeError('Browser state refresh returned no tokens')\n"
            stub_class_code += "        self._pytincture_replay_pool.extend(tokens)\n"
            ...
            stub_class_code += "    def _abort_bff_request(self, controller):\n"
            stub_class_code += "        try:\n"
            stub_class_code += "            controller.abort()\n"
            stub_class_code += "        except Exception:\n"
            stub_class_code += "            pass\n"
            stub_class_code += "    async def _close_bff_stream(self, reader, controller):\n"
            stub_class_code += "        self._abort_bff_request(controller)\n"
            stub_class_code += "        if reader is None:\n"
            stub_class_code += "            return\n"
            stub_class_code += "        try:\n"
            stub_class_code += "            await asyncio.wait_for(reader.cancel(), timeout=1.0)\n"
            stub_class_code += "        except (asyncio.CancelledError, Exception):\n"
            stub_class_code += "            pass\n"
            stub_class_code += "    async def _request_pytincture_state(self):\n"
            stub_class_code += "        if not self._pytincture_replay_enabled:\n"
            stub_class_code += "            return\n"
            stub_class_code += "        from js import AbortController, fetch\n"
            stub_class_code += "        from pyodide.ffi import to_js\n"
            stub_class_code += "        controller = AbortController.new()\n"
            ...
            stub_class_code += "        options['signal'] = controller.signal\n"
            stub_class_code += "        try:\n"
            ...
            stub_class_code += "            tokens = self._decode_pytincture_state(encoded)\n"
            stub_class_code += "            if not tokens:\n"
            stub_class_code += "                raise RuntimeError('Browser state refresh returned no tokens')\n"
            stub_class_code += "            self._pytincture_replay_pool.extend(tokens)\n"
            ...
            stub_class_code += "            def clear_refill(completed):\n"
            stub_class_code += "                if owner._pytincture_replay_refill_task is completed:\n"
            stub_class_code += "                    owner._pytincture_replay_refill_task = None\n"
            stub_class_code += "                if not completed.cancelled():\n"
            stub_class_code += "                    try:\n"
            stub_class_code += "                        completed.exception()\n"
            stub_class_code += "                    except Exception:\n"
            stub_class_code += "                        pass\n"
            stub_class_code += "            task.add_done_callback(clear_refill)\n"
            stub_class_code += "        await asyncio.shield(task)\n"
            ...
            stub_class_code += "        if not self._pytincture_replay_enabled:\n"
            stub_class_code += "            return ''\n"
            stub_class_code += "        while not self._pytincture_replay_pool:\n"
            ...
            stub_class_code += f"        from js import AbortController, fetch, window\n"
            ...
            if streaming_methods:
                ...
                stub_class_code += f"        from js import AbortController, fetch, TextDecoder\n"
                stub_class_code += f"        from pyodide.ffi import to_js\n"
                stub_class_code += "        controller = AbortController.new()\n"
                stub_class_code += "        reader = None\n"
                stub_class_code += f"        options = {{'method': method, 'headers': {{'Content-Type': 'application/json'}}}}\n"
                stub_class_code += "        replay_token = await self._take_pytincture_state()\n"
                stub_class_code += f"        if method != 'GET':\n"
                stub_class_code += f"            options['headers']['X-CSRF-Token'] = self._csrf_token()\n"
                ...
                stub_class_code += "        finally:\n"
                stub_class_code += "            await self._close_bff_stream(reader, controller)\n"
